...
var TabCollection = function(controller) {
	...
	this.next = function () {
		if (self.__iterator) self.__iterator = self.__iterator.next;
		return self.__iterator;
	};

	this.prev = function () {
		if (self.__iterator) self.__iterator = self.__iterator.prev;
		return self.__iterator;
	};
	...
	this.to_index = function(index) {
		...
		if (index <= 0) return self.rewind();
		else if (index >= (self.count() - 1)) return self.prewind();
	...
	this.remove = function (index) {
		if (self.to_index(index) && self.valid() || (typeof index == 'undefined' && self.valid())) {
			if (index == 0) self._head = self.current().next;
			if (index == self.count() - 1) self._tail = self.current().prev;

			if (self.current().prev !== null) self.current().prev.next = self.current().next;
			if (self.current().next !== null) self.current().next.prev = self.current().prev;
			self._length--;
		}
	...
	this.clear = function (callback) {
		while (self.rewind() !== null) self.remove(0);
		return (callback instanceof Function) ? callback() : self;
	};
